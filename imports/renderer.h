    ...
    *(d_list + 3) = new sphere(point3(4.0f, 1.0f, 0.0f), 1.0f, *(mat_list+3));
    ...
    for (int a = -11; a < 11; a++) {
        for (int b = -11; b < 11; b++) {
            ...
            if (choose_mat < 0.8f) {
                // diffuse
                color albedo = random_vec(0, 1, &local_rand_state) * random_vec(0, 1, &local_rand_state);
                *(mat_list + obj_index) = new lambertian(albedo);
                point3 center2 = center + vec3(0, random_float(0.0f, 0.5f, &local_rand_state), 0);
                *(d_list + obj_index) = new sphere(center, center2, 0.2, *(mat_list + obj_index));
                // *(d_list + obj_index) = new sphere(center, 0.2, *(mat_list + obj_index));
            } 
            else if (choose_mat < 0.95f) {
                // metal
                color albedo = random_vec(0.5, 1, &local_rand_state);
...
__host__ void render(camera* h_cam, ostream& destination){
    ...
    clog<<"total pixels: "<<h_cam->image_w() * h_cam->image_h()<<flush;
