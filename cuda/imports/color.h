...
__host__ __device__ inline float linear_to_gamma(float d){
...
__host__ __device__ inline int float_to_color(float d, interval i) {
    return (int) (i.clamp(d) * (float) 255.999);
}

//writes individual pixel to the stream out
__host__ __device__ void write_color(int *fb, int pixel_index, const color& c){
